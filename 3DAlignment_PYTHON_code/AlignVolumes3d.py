...
def fastAlignment3D(sym,vol1,vol2,n,Nprojs=30,trueR=None,G_group=None,refrot=0,verbose=0):
    ...
    if refrot == 1:
        ...
        opt.trueRots = trueR_tild; opt.trueRots_J = trueR_tild_J;         
...
def AlignVolumes(vol1,vol2,verbose=0,opt=None):
    ...
    if hasattr(opt,'trueR'): trueR = opt.trueR
    else: trueR = None
    ...
    if no2 > no1:
        J3 = np.diag([1, 1, -1]) 
        corr_v = no2
        R_est = R_est_J; R_est = J3 @ R_est @ J3; 
        estdx_ds = estdx_J_ds
    ...
    if refrot == 1 and G_flag == 0 and sym_flag == 1:  
        ...
        err = np.min(dist.ravel()); row = np.array(np.where(dist == err))[0];
        O = Rots[:,:,row[0]]    
        # BFGS optimization:
